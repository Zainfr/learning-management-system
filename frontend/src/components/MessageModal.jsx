...
const MessageModal = ({ show, handleClose, title, message, success }) => {
    ...
    return (
        <>
            ...
            {/* Modal */}
            <div
                className={`fixed inset-0 flex items-center justify-center z-50 transition-transform duration-300 ease-out ${show ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`}
            >
                <div className="bg-white rounded-lg shadow-lg p-6 max-w-sm w-full transform transition-all">
                    ...
                    <div className="flex justify-end mt-6">
                        <button
                            onClick={handleClose}
                            className={`px-4 py-2 rounded ${success ? 'bg-green-500 hover:bg-green-600' : 'bg-red-500 hover:bg-red-600'} text-white`}
                        >
                            {success ? 'Okay' : 'Close'}
                        </button>
                    </div>
